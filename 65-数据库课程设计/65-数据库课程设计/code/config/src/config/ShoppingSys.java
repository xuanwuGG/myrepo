...
public class ShoppingSys extends JFrame
{
	...
	public ShoppingSys() 
	{
		...
		String [][]shopStringssss=new String[50][];
		for(int j=0;j<30;j++)
		{
		...
		JLabel biaoyuJLabel=new JLabel("急速快递"+",使命必达!");
		...
		tables2.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				int p=0;
				p=tables2.getSelectedRow();
				Connection co0=null;
				PreparedStatement ps0=null;
				ResultSet rs0=null;
				String sql0="select * from 订单 where 订单ID=?";
				try
				{
					co0=Conn.getConnection();
					ps0=co0.prepareStatement(sql0);
					ps0.setString(1,shopStringssss[tables2.getSelectedRow()][0]);
					rs0=ps0.executeQuery();
					if(rs0.next())
					{
						qianshouren.setText("签收人:"+rs0.getString("货主名称"));
						qianshoudidian.setText("签收地址:"+rs0.getString("货主地址"));
					}
					else
					{
						qianshouren.setText("签收人:"+"XXX");
						qianshoudidian.setText("签收地点:"+"XXXX");
					}
				}catch(Exception e1) {
					e1.printStackTrace();
				}
				...
				querneBox.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						if(querneBox.isSelected()&&shopStringssss[tables2.getSelectedRow()][0]!=null)
						{
							Connection co=null;
							PreparedStatement ps=null;
							String sql="update 订单 set 到货日期=? where 订单ID=?";
							try
							{
								co=Conn.getConnection();
								ps=co.prepareStatement(sql);
								ps.setDate(1,time);
								ps.setString(2,shopStringssss[tables2.getSelectedRow()][0]);
								ps.executeUpdate();
							}catch(Exception e1) {
								e1.printStackTrace();
							}
							...
							querneBox.setSelected(false);
							for(int m=0;m<50;m++)
							{
								shopStringssss[m]=new String[4];
							}
							tableModels2.setDataVector(shopStringssss, s2);
							Connection co1=null;
							ResultSet rs=null;
							PreparedStatement ps1=null;
							String sql1="select * from 订单,订单明细,产品 where 到货日期 is null and 订单明细.订单ID=订单.订单ID and 订单明细.产品ID=产品.产品ID order by 订单.订单ID";
							try
							{
								co1=Conn.getConnection();
								ps1=co1.prepareStatement(sql1);
								rs=ps1.executeQuery();
								int k=0;
								while(rs.next())
								{
									shopStringssss[k][0]=rs.getString("订单ID");
									shopStringssss[k][1]="急速快递";
									shopStringssss[k][2]=rs.getString("产品名称");
									shopStringssss[k][3]=rs.getString("货主地址");
									k++;
								}
								tableModels2.setDataVector(shopStringssss, s2);
								qianshouren.setText("签收人:"+"XXX");
							...
							Connection co11=null;
							PreparedStatement ps11=null;
							ResultSet rs1=null;
							String sql11="select * from 订单,订单明细,产品 where 到货日期 is not null and 订单明细.订单ID=订单.订单ID and 订单明细.产品ID=产品.产品ID order by 订单.订单ID";
							int p=0;
							try 
							{
								co11=Conn.getConnection();
								ps11=co11.prepareStatement(sql11);
								rs1=ps11.executeQuery();
								while(rs1.next())
								{
									p++;
								}
								finishButton.setText("已完成订单"+"("+p+")");
							}catch(Exception e1)
							{
								e1.printStackTrace();
							}
							Connection co111=null;
							ResultSet rs11=null;
							PreparedStatement ps111=null;
							String sql111="select * from 订单,订单明细,产品 where 到货日期 is null and 订单明细.订单ID=订单.订单ID and 订单明细.产品ID=产品.产品ID order by 订单.订单ID";
							try
							{
								co111=Conn.getConnection();
								ps111=co111.prepareStatement(sql111);
								rs11=ps111.executeQuery();
								int k=0;
								while(rs11.next())
								{
									k++;
								}
								doingButton.setText("未完成订单"+"("+k+")");
							}catch(Exception e1)
							{
								e1.printStackTrace();
							}
		...
		finishButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				ResultSet rs=null;
				PreparedStatement ps=null;
				String sql="select * from 订单,订单明细,产品 where 到货日期 is not null and 订单明细.订单ID=订单.订单ID and 订单明细.产品ID=产品.产品ID order by 订单.订单ID";
				try
				{
					co=Conn.getConnection();
					ps=co.prepareStatement(sql);
					...
					while(rs.next())
					{
						shopStringsss[k][0]=rs.getString("订单ID");
						shopStringsss[k][1]=rs.getString("货主名称");
						shopStringsss[k][2]=rs.getString("产品名称");
						shopStringsss[k][3]=rs.getString("数量");
						k++;
					}
				}catch(Exception e1)
				{
				...
				DefaultTableModel tableModels2=new DefaultTableModel(shopStringsss,s1) {
					public boolean isCellEditable(int row,int column)
					{
						return false;
					}
		...
		doingButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				ResultSet rs=null;
				PreparedStatement ps=null;
				String sql="select * from 订单,订单明细,产品 where 到货日期 is null and 订单明细.订单ID=订单.订单ID and 订单明细.产品ID=产品.产品ID order by 订单.订单ID";
				try
				{
					co=Conn.getConnection();
					ps=co.prepareStatement(sql);
	   ...
	   //jp3 definition
		...
		//jpd declaration
				...
				qianshoudidian.setBounds(480,300,300,40);
				...
				biaoyuJLabel.setBounds(150,90,600,60);
		...
		//jps declaration
				...
				proidJLabel.setFont(f1Font);pronameJLabel.setFont(f1Font);prostopJLabel.setFont(f1Font);buydate.setFont(f1Font);senddate.setFont(f1Font);senButton.setFont(jFont);
				...
				jps.add(yourinfo);jps.add(yourname);jps.add(yourdegree);jps.add(scros1);jps.add(statusJLabel);jps.add(buydate);
...
//  	    r2.get();
//	  	ImageIcon []icon11= {
//	  			new ImageIcon("调味品.jpg"),
//	  			new ImageIcon("饮料.jpg"),
//	  	};
  	    read[] r=new read[10];
  	    for(int k=0;k<10;k++)
  	    {
  	    ...
  	    ImageIcon[] icon11=new ImageIcon[2];
  	    Connection conn1=null;
  	    PreparedStatement ps1=null;
  	    ResultSet rs1=null;
  	    String sql1="select * from 类别";
  	    try
  	    {  	
  	    	...
  	    	while(rs1.next())
  	    	{	
  	    		icon11[p]=new ImageIcon(rs1.getString("类别名称")+".jpg");
  	    		icon11[p].setImage(icon11[p].getImage().getScaledInstance(320, 280,Image.SCALE_DEFAULT));
  	    		p++;
  	    	}
  	    }catch(Exception e1) {
  	    	e1.printStackTrace();
  	    }
//	  	icon11[0].setImage(icon11[0].getImage().getScaledInstance(320, 280,Image.SCALE_DEFAULT));
   		class Myjpanel extends JPanel{
   		    public void paint(Graphics g) {
   		        super.paint(g);
   		        g.drawImage(icon11[index%icon11.length].getImage(), 0, 0,this);
   		        index++;
   		    }}
...
//   	    icon11.setImage(icon11.getImage().getScaledInstance(320, 280,Image.SCALE_DEFAULT));
//   	    label.setIcon(icon11);
   	    Myjpanel mp=new Myjpanel();
   	    mp.setBounds(40, 30, 320, 280);
  	 ...
  	 String []s11= {"订单ID","顾客姓名","需求产品","需求数量"};
		DefaultTableModel tableModels11=new DefaultTableModel(shopStringsss,s11) {
			public boolean isCellEditable(int row,int column)
			{
				return false;
			}
		};
		String[] da1=new String[100];
  	 supplyJLabel.addMouseListener(new MouseListener() {
		...
		public void mouseClicked(MouseEvent e) {
			...
			Connection conn=null;
				...
				Connection conn1=null;
	 			...
	 			try
	 			{
	 				...
	 				{	
	 					...
	 					da1[k]=resultSet1.getString("订购日期");	 		
	 					k++;
	 				}
	 				tableModels11.setDataVector(shopStringsss, s11);
	 			}catch(Exception e7){
	 				e7.printStackTrace();
	 			}
 			}catch(Exception e6){
 			...
 			JTable tables1 =new JTable(tableModels11);
 			tables1.setRowHeight(26);
 			JScrollPane scros1=new JScrollPane(tables1);
 			JLabel statusJLabel=new JLabel("当前状态:请选中订单后执行操作");
 			JLabel buydate=new JLabel("购买时间:暂无");
 			...
 			statusJLabel.setBounds(210,320,340,50);
 			statusJLabel.setFont(jFont);
 			buydate.setBounds(210,380,230,50);
 			senddate.setBounds(400,380,340,50);
 			senButton.setBounds(600,380,140,40);
 			setstopButton.setBounds(400,500,140,40);
 			proidJLabel.setBounds(210,440,60,50);
 					...
 					confirmchangeButton.addActionListener(new ActionListener() {
 						public void actionPerformed(ActionEvent e) {
 							...
 							Connection conn1=null;
 				 			...
 				 			for(int m=0;m<30;m++)
 				 			{
 				 				shopStringsss[m]=new String[4];
 				 			}
 				 			for(int m=00;m<100;m++)
 				 			{
 				 				da1[m]=null;
 				 			}
 				 			String sql1="select * from 订单,订单明细,产品 where 发货日期 is null and 订单明细.订单ID=订单.订单ID and 订单明细.产品ID=产品.产品ID";
 			...
 			tables1.addMouseListener(new MouseListener() {
 				...
 				public void mouseClicked(MouseEvent e) {
 					...
 					if(da1[p] == null)
 					{
 	 					buydate.setText("购买时间:暂无");
 	 					senddate.setText("发货时间:暂无");
 	 					statusJLabel.setText("当前状态:请选中订单后执行操作");
 					}
 					else
 					{
 						buydate.setText("购买时间:"+da1[p]);
			...
			PreparedStatement ps0=null;
			ResultSet rs0=null;
			String sql0="select * from 订单 where 发货日期 is null";
			int a=0;
			try
			{
				co0=Conn.getConnection();
    	   ...
    	   public void itemStateChanged(ItemEvent e) {
   				...
   				public void actionPerformed(ActionEvent e) {
   					...
   					try {
   		        		...
   		        		String sql3="SELECT * FROM 客户 where  客户ID=?;";
    				        ...
    				        if(resultSet.next()){};
    			    		String sql2= "insert into 订单(订单ID,订购日期,发货日期,到货日期,货款确认日期,货主名称,货主地址,货主城市,货主地区,货主邮政编码,货主国家) values (?,?,?,?,?,?,?,?,?,?,?);";
    			        	PreparedStatement ps1=null;
    			        	ps1=conn.prepareStatement(sql2);
    		        		ps1.setInt(1,id);
    		        		ps1.setDate(2, time);
    		   ...
    		   shanchu.addActionListener(new ActionListener() {
    			...
    			public void mouseExited(MouseEvent e) {}
    	  	   ...
    	  	   jp5.addMouseListener(new MouseListener() {
    			public void mouseReleased(MouseEvent e) {}
    			public void mousePressed(MouseEvent e) {}
    			public void mouseExited(MouseEvent e) {}
    			public void mouseEntered(MouseEvent e) {}
